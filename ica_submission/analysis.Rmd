...
```{r include = T, echo = F, out.width = "100%", fig.cap = ""}
geo_calcs %>% 
  ...
  labs(title = "Average Percentage Contributed to Republicans by Geographic Category",
...
```{r}
donor_geography %>% 
  group_by(county, walker_per) %>% 
  summarize(per_rep = mean(per_rep), geo_category) %>% 
  ...
  labs(title = "WI Counties Vote % for Walker (Republican) & 
       ...
       y = "2020 Average % Contributed to Republicans",
