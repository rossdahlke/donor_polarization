...
polls <- tibble(date = seq.Date(as.Date("2002-01-01"), as.Date("2020-12-31"), 1)) %>% 
  left_join(read_csv("../data/polling_data.csv") %>% 
  mutate(date = lubridate::mdy(date))) 

polls_interpolated <- tibble(pollster = polls$pollster,
       date = polls$date,
...
polls_interpolated %>% 
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 5L), breaks = seq(0, 1, .05)) +
...
polls_interpolated %>% 
  ...
  filter(pollster != "wis_survey") %>% 
  mutate(neither = neither / 100,
         imputed_neither = imputed_neither / 100) %>% 
  ggplot(aes(x = date, color = pollster)) +
  geom_smooth(aes(y = imputed_neither), color = "darkgrey", span = .2, method = "lm")  +
  geom_point(aes(y = neither), color = "black") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 5L), breaks = seq(0, 1, .05)) +
  scale_x_date(breaks = "1 year", date_labels = "%Y") +
  labs(title = "Wisconsin Party Independent/ Neither Party Identification",
...
polls_interpolated %>% 
  ...
  mutate(neither = neither / 100,
         imputed_neither = imputed_neither / 100) %>% 
  ggplot(aes(x = date, color = pollster, group = pollster)) +
  geom_smooth(aes(y = imputed_neither, color = pollster), span = .5)  +
  geom_point(aes(y = neither)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 5L), breaks = seq(0, 1, .05)) +
  scale_x_date(breaks = "1 year", date_labels = "%Y") +
  labs(title = "Wisconsin Party Independent/ Neither Party Identification",
       subtitle = "Imputed values",
...
modularity_window <- function(end_date,
  ...
  nodes_w_party <- by_donor %>% 
    ...
    mutate(party_num = if_else(is.na(party_num), 4, party_num))
  ...
  print(start_date)
  print(end_date)
  print(gubernatorial_donations)
  print(modularity)
  
  return(tibble(start_date = start_date,
                end_date = end_date,
                includes_gubernatorial = gubernatorial_donations,
...
```{r}
modularity_calcs <- foreach(end_date = seq.Date(as.Date("2009-12-31"), as.Date("2014-12-31"), 365),
                           .packages = c("dplyr", "tidyr", "igraph"),
                           .errorhandling = c("pass"),
                           .combine = "rbind") %:%
  foreach(gubernatorial_donations = c(F),
          .packages = c("dplyr", "tidyr", "igraph"),
          .errorhandling = c("pass"),
          .combine = "rbind") %dopar%
...
```{r}
modularity_calcs %>% 
  ggplot(aes(end_date, modularity, group = includes_gubernatorial, color = includes_gubernatorial)) +
  geom_line() +
  facet_wrap(.~includes_gubernatorial)
```

```{r}
by_year <- modularity_calcs %>% 
  mutate(year = lubridate::year(end_date)) %>% 
  left_join(polls %>% 
  filter(!is.na(pollster)) %>% 
  mutate(year = lubridate::year(date)) %>% 
  group_by(year) %>% 
  summarize(neither_avg = mean(neither))) %>% 
  ungroup() %>% 
  mutate(lead_modularity = lead(modularity))

cor.test(by_year$modularity, by_year$neither_avg)
```
```{r}
cor.test(by_year$lead_modularity, by_year$neither_avg)
```


```{r}
by_year %>% 
  ggplot(aes(modularity, neither_avg)) +
  geom_point() +
...
```{r}
sen_files <- dir("../data/state_senate_donations/", pattern = "*.csv") %>% 
  map(~ read_csv(file.path("../data/state_senate_donations/", .))) %>% 
  reduce(rbind)

ass_files <- dir("../data/state_assembly_donations/", pattern = "*.csv") %>% 
  map(~ read_csv(file.path("../data/state_assembly_donations/", .))) %>% 
  reduce(rbind)

all_donations <- sen_files %>% 
  rbind(ass_files) %>% 
  janitor::clean_names()
```
